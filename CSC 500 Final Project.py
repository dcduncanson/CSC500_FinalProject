...
class ShoppingCart:
    ...
    def change_quantity(self, item: ItemToPurchase):
        for cart_item in self.cart_items:
            if cart_item.item_name == item.item_name:
                cart_item.item_quantity = item.item_quantity
                print(f"Quantity of {item.item_name} updated to {item.item_quantity}.")
                return
        print("Item not found in cart. Quantity not updated.")
...
def print_menu(cart):
    while True:
        ...
        print('c - Change item quantity in cart')
